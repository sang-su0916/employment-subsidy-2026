...
function loadBuiltinData() {
    ...
    // SUBSIDY_DATA 또는 SUBSIDIES가 전역에 정의되어 있다고 가정
    if (typeof SUBSIDY_DATA !== 'undefined') {
        builtinSubsidies = {
            version: SUBSIDY_DATA.version || '2.0.0-builtin',
            lastUpdated: SUBSIDY_DATA.lastUpdated || '2026-01-11',
            description: '내장 2026년 고용지원금 데이터',
            subsidies: SUBSIDY_DATA.subsidies,
            companySizeCategories: typeof COMPANY_SIZE_CATEGORIES !== 'undefined' ? COMPANY_SIZE_CATEGORIES : {},
            industryTypes: typeof INDUSTRY_TYPES !== 'undefined' ? INDUSTRY_TYPES : []
        };
        console.log('✅ 내장 데이터 로드 성공 (SUBSIDY_DATA)');
        return builtinSubsidies;
    }

    // 레거시 호환: SUBSIDIES_2026 배열 직접 참조
    if (typeof SUBSIDIES_2026 !== 'undefined') {
        ...
        console.log('✅ 내장 데이터 로드 성공 (SUBSIDIES_2026)');
        return builtinSubsidies;
    }

    // SUBSIDIES 배열 참조 (최후 수단)
    if (typeof SUBSIDIES !== 'undefined') {
        builtinSubsidies = {
            version: '2.0.0-builtin',
            lastUpdated: '2026-01-11',
            description: '내장 2026년 고용지원금 데이터',
            subsidies: SUBSIDIES,
            companySizeCategories: typeof COMPANY_SIZE_CATEGORIES !== 'undefined' ? COMPANY_SIZE_CATEGORIES : {},
        ...
        console.log('✅ 내장 데이터 로드 성공 (SUBSIDIES)');
        return builtinSubsidies;
    }
